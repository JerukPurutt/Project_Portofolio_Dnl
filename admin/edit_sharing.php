...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $id_sharing = (int)$_POST['id_sharing'];
    ...
    if (isset($_FILES['sharing_file']) && $_FILES['sharing_file']['error'] == 0 && $_FILES['sharing_file']['size'] > 0) {
        ...
        $sql_update = "UPDATE tb_sharing SET 
                        judul_experience = ?, link_optional = ?, deskripsi = ?, 
                        gambar_data = ?, gambar_tipe = ?
                       WHERE id_sharing = ?";

        $stmt_update = $koneksi->prepare($sql_update);
        // Tipe: s(judul), s(link), s(desk), b(blob), s(tipe), i(id)
        $stmt_update->bind_param("sssbsi", $judul, $link, $deskripsi, $gambar_data, $gambar_tipe, $id_sharing);
    } else {
        // --- TIDAK ADA FILE BARU ---
        ...
        $stmt_update = $koneksi->prepare($sql_update);
        // Tipe: s(judul), s(link), s(desk), i(id)
        $stmt_update->bind_param("sssi", $judul, $link, $deskripsi, $id_sharing);
    }
...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    echo "Error: ID Sharing tidak ditemukan.";
    exit;
}
...
$sql_select = "SELECT * FROM tb_sharing WHERE id_sharing = ?";
$stmt_select = $koneksi->prepare($sql_select);
$stmt_select->bind_param("i", $id_sharing);
$stmt_select->execute();
...
if (!$sharing) {
    echo "Error: Data sharing tidak ditemukan.";
    $stmt_select->close();
    $koneksi->close();
    exit;
}
...
<head>
    ...
    <title>Edit Sharing - Daniel</title>
        ...
        <div class="flex-1 h-full overflow-y-auto p-12">
            <div class="max-w-5xl">
                <div class="bg-black rounded-3xl p-12">
                    ...
                    <div class="mb-8">
                        <h2 class="text-5xl font-bold text-lime-400 mb-2">Edit Sharing</h2>
                        <p class="text-gray-400">Perbarui postingan:
                            <strong><?php echo htmlspecialchars($sharing['judul_experience']); ?></strong>
                        </p>
                    </div>

                    <form action="edit_sharing.php" method="POST" enctype="multipart/form-data">

                        <input type="hidden" name="id_sharing" value="<?php echo $sharing['id_sharing']; ?>">
                        ...
                        <div class="mb-6">
                            <input type="text" id="sharing_title" name="sharing_title"
                                ...
                                value="<?php echo htmlspecialchars($sharing['judul_experience']); ?>"
                        ...
                        <div class="mb-6">
                            <input type="url" id="sharing_link" name="sharing_link"
                                ...
                                value="<?php echo htmlspecialchars($sharing['link_optional']); ?>"
                                class="w-full bg-gray-900 text-white text-sm h-12 border border-gray-700 p-4 rounded-xl 
                                       focus:outline-none focus:border-lime-400 placeholder-gray-500">
                        </div>
                        ...
                        <div class="mb-8">
                            <textarea id="sharing_description" name="sharing_description" rows="5"
                                ...
                                class="w-full bg-gray-900 text-white border text-sm border-gray-700 p-4 rounded-xl 
                                       focus:outline-none focus:border-lime-400 placeholder-gray-500 resize-none"><?php echo htmlspecialchars($sharing['deskripsi']); ?></textarea>
                        </div>
                    ...
                    <div class="text-center mt-8">
                        <a href="sharing.php" class="text-gray-400 hover:text-white transition">← Kembali ke Daftar Sharing</a>
                    </div>
